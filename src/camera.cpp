...
ImagePtr acquisition::Camera::grab_frame(bool& success)
{
  ImagePtr pResultImage = pCam_->GetNextImage(GET_NEXT_IMAGE_TIMEOUT_);

  if (pResultImage->IsIncomplete())
  {
    ROS_WARN_STREAM("Image incomplete from camera " << get_id() << " with image status " << pResultImage->GetImageStatus() << "!");
  }
  else
  {
    ...
    {
      frameID_ = pResultImage->GetFrameID();
      //ROS_ASSERT_MSG(frameID_ == 0 ,"First frame ID was not zero! Might cause sync issues later..."); //Sean edit
    }

    //ROS_INFO_STREAM("Grabbed frame " << frameID_ << " from camera " << get_id() << " with timestamp " << timestamp_*1000);
...
{
  bool success = false;
  ImagePtr pResultImage = grab_frame(success);
...
Mat acquisition::Camera::convert_to_mat(ImagePtr pImage)
{
  ...
  {
    convertedImage = pImage->Convert(PixelFormat_Mono16); //, NEAREST_NEIGHBOR);
  }
  ...
  //image data contains padding. When allocating Mat container size, need to account for the X,Y image data padding.
  Mat img;
  if (COLOR_)
  {
  ...
  {
    img = Mat(colsize + YPadding, rowsize + XPadding, CV_16UC1, convertedImage->GetData(), convertedImage->GetStride());
  }
...
bool acquisition::Camera::grab_image(sensor_msgs::ImagePtr image)
{
  bool success = false;
  ImagePtr image_ptr = grab_frame(success);
  if (!success)
  {
    return false;
  }

  uint8_t bitsPerPixel = image_ptr->GetBitsPerPixel();

  std::string imageEncoding = sensor_msgs::image_encodings::MONO8;
  PixelFormatEnums pixel_format = image_ptr->GetPixelFormat();

  //ROS_INFO("Pixel format: %s", image_ptr->GetPixelFormatName().c_str());
  ...
  fillImage(*image, imageEncoding, image_ptr->GetHeight() + image_ptr->GetYPadding(),
    image_ptr->GetWidth() + image_ptr->GetXPadding(),
    image_ptr->GetStride(), image_ptr->GetData());
